...
public class UpdateUserController implements Controller {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response)	throws Exception {
    	...
    	if (request.getMethod().equals("GET")) {	
			...
			UserInfoDTO user = userInfoService.findUserInfo(updateId);	// 수정하려는 사용자 정보 검색
			...
			if (UserSessionUtils.isLoginUser(updateId, session) ||
				...
				return "/mypage/profileUpdate.jsp";   // 검색한 사용자 정보 및 커뮤니티 리스트를 updateForm으로 전송     
			}    
			...
			return "/mypage/profile.jsp";	// 사용자 보기 화면으로 이동 (forwarding)
	    }	
		...
		manager.updateUserInfo(updateUser);		
